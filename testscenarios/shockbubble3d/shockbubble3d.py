...
def qinit(state,x0=0.5,y0=0.,z0=0.,r0=0.2,rhoin=0.1,pinf=5.):
    grid = state.grid
    ...
    for i in xrange(state.q.shape[3]):
      state.q[0,:,:,i] = rhoin*(r<=r0) + rhoout*(r>r0)
      state.q[1,:,:,i] = 0.
      state.q[2,:,:,i] = 0.
      state.q[3,:,:,i] = (pin*(r<=r0) + pout*(r>r0))/gamma1
      state.q[4,:,:,i] = 1.*(r<=r0)
...
def shockbubble(use_petsc=False,kernel_language='Fortran',solver_type='classic',iplot=False,htmlplot=False,amr_type=None):
    ...
    z = pyclaw.Dimension('z',0.0,0.5,mz)
