...
peanoclaw::grid::plotter::VTUSubgridPlotter::VTUSubgridPlotter(
  std::string fileName,
...
//    _gap(0.015),
  ...
  _vertexWriter     = _vtuWriter.createVertexWriter();
  _cellWriter       = _vtuWriter.createCellWriter();

  for(int i = 0; i < unknownsPerSubcell; i++) {
    if(_plotQ.empty() || _plotQ.find(i) != _plotQ.end()) {
      std::stringstream stringstream;
      stringstream << "q" << i;
      _cellQWriter.push_back( _vtuWriter.createCellDataWriter(stringstream.str(), 1) );
    }
  }
  for(int i = 0; i < parametersWithoutGhostlayerPerSubcell; i++) {
    if(_plotParameterWithoutGhostlayer.empty() || _plotParameterWithoutGhostlayer.find(i) != _plotParameterWithoutGhostlayer.end()) {
      std::stringstream stringstream;
      stringstream << "pWoG" << i;
      _cellParameterWithoutGhostlayerWriter.push_back( _vtuWriter.createCellDataWriter(stringstream.str(), 1) );
    }
  }
  for(int i = 0; i < parametersWithGhostlayerPerSubcell; i++) {
    if(_plotParameterWithGhostlayer.empty() || _plotParameterWithGhostlayer.find(i) != _plotParameterWithGhostlayer.end()) {
      std::stringstream stringstream;
      stringstream << "pWG" << i;
      _cellParameterWithGhostlayerWriter.push_back( _vtuWriter.createCellDataWriter(stringstream.str(), 1) );
    }
  }

  if(_plotMetainformation) {
    _cellSubdivisionFactorWriter = _vtuWriter.createCellDataWriter("SubdivisionFactor",1);
    _cellGhostLayerWidthWriter   = _vtuWriter.createCellDataWriter("GhostlayerWidth",1);
    _cellTimeOldWriter           = _vtuWriter.createCellDataWriter("timeOld", 1);
...
peanoclaw::grid::plotter::VTUSubgridPlotter::~VTUSubgridPlotter() {
  ...
  _vtuWriter.writeToFile( _fileName );
...
void peanoclaw::grid::plotter::VTUSubgridPlotter::plotSubcell(
  const Patch&                            patch,
...
tarch::la::Vector<DIMENSIONS, double> peanoclaw::grid::plotter::VTUSubgridPlotter::computeGradient(
  const Patch&                            patch,
  const peanoclaw::grid::SubgridAccessor& accessor,
  tarch::la::Vector<DIMENSIONS, int>      subcellIndex,
  int                                     unknown
...
void peanoclaw::grid::plotter::VTUSubgridPlotter::plotSubgrid(
  const Patch& subgrid
) {
...
void peanoclaw::grid::plotter::VTUSubgridPlotter::close() {
  _vertexWriter->close();
  _cellWriter->close();
